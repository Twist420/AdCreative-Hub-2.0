...
export const useScheduleInsights = ({ schedules, requirements, deliverySets, todayDateString = '2026-06-26' }) => {
  const getScheduleRequirements = (scheduleId) =>
    requirements.value.filter((requirement) => requirement.scheduleId === scheduleId)

  const scheduleInsights = computed(() => {
    ...
    ;(schedules?.value || []).forEach((schedule) => {
      ...
      const notStartedRequirements = relatedRequirements.filter((req) =>
        req.prodStatus === 'Unscheduled' || req.prodStatus === 'Scheduled' || !req.prodStatus,
      )
      ...
      const quickRequirementIds = relatedRequirements
        ...
        .map((req) => req.id)
      const draftSetCount = (deliverySets?.value || []).filter(
      ...
      if (schedule.rolloverStatus === 'Closed') {
        ...
        status = '全部完成且未投放'
      } else if (relatedRequirements.length === 0 || notStartedRequirements.length === relatedRequirements.length) {
        status = '完全未开始'
      } else if (completedRequirements.length > 0 || schedule.rolloverStatus === 'PartialCompleted' || schedule.inheritedToScheduleIds?.length) {
        status = '部分完成'
  ...
  const getScheduleInsight = (scheduleId) => {
    return scheduleInsights.value.get(scheduleId) || null
  }
