...
export const generateSchedules = () => {
  ...
  const priorities = ['Low', 'Mid', 'High', 'Highest']
  const difficulties = ['Senior', 'Junior', 'Test']
  const forms = ['Video', 'Playable', 'Image']
  const scenarios = ['Standard', 'Localized', 'ASO']
  const types = ['Original-Gameplay', 'Original-Hook', 'Scaling-Iteration']
  const scheduleWeeks = [
  ...
  return scheduleWeeks.map((week, index) => {
    ...
    const directionText = ['展示新玩法', '角色互动', '场景融合', '吸量大字报', '3D剧情'][index % 5]
    ...
    const scheduleId = `sched-${index}`
    const rolloverMeta = index === 12
      ? {
    ...
    return {
      ...
      directionName: `方向${index + 1}: ${directionText}`,
      ...
      requirementStart: week.start,
      requirementEnd: week.end,
      productionEnd: week.end,
      validationGoal: `验证${directionText}转化表现与Cvr`,
      directionTags: directionTagPresets[index % directionTagPresets.length],
      broadDirection: index % 5 === 3 ? '大字报' : index % 5 === 4 ? '3D玩法' : '原始玩法',
      materialStage: index % 3 === 0 ? '新' : index % 3 === 1 ? '老' : '迭',
      channels: [index % 2 === 0 ? 'all' : 'fb'],
      campaign: `global_android_cam_${100 + index}`,
      submissionDeadline: week.end,
      acceptanceDate: week.start,
      ...rolloverMeta,
    }
  })
}
...
export const generateRequirements = () => {
  ...
  const reqStatuses = ['Approved', 'Pending', 'Draft', 'Modification']
  const prodStatuses = ['Scheduled', 'InProgress', 'Completed']
  const deliveryStatuses = ['NotLaunched', 'Delivering', 'Paused']
  const priorities = ['Low', 'Mid', 'High', 'Highest']
  const creators = ['唐欣怡', '吉意煊', '马嘉良']
  const producers = ['张欢', '吴楠', '曲冬丽', '刘洋', '孙崇洋', '张永进']
  const materialStages = ['新', '老', '迭']
  const broadDirections = ['大字报', '原始玩法', '3D玩法']
  const languages = ['en', 'zh', 'jp', 'kr']
  const channels = ['all', 'apl', 'fb', 'uac']
  const testStatuses = ['待测试', '测试中', '数据合格', '不达标']

  const buildRequirement = ({ index, assetIndex, assetVersion, scheduleId, parentId }) => {
    const typeVal = index % 3 === 0 ? 'Playable' : index % 3 === 1 ? 'Video' : 'Image'
  ...
  const directionRequirements = sched12VersionPlan.flatMap((group) =>
    group.versions.map((version) => {
      ...
      directionRequirementsSeedIndex += 1
      return requirement
    }),
  )

  const otherRequirements = Array.from({ length: 18 }).map((_, index) => {
    const fallbackSchedules = schedules.filter((schedule) => schedule.id !== 'sched-12')
    const scheduleId = index <= 2
      ? 'sched-2'
      : index <= 4
        ? 'sched-14'
        : index === 5
    ...
    return buildRequirement({
      index: index + directionRequirements.length,
      assetIndex: 3368 - index,
      assetVersion: '01',
      scheduleId,
    })
  })
...
const daysBetween = (start, end) => {
  ...
  if (Number.isNaN(startTime) || Number.isNaN(endTime)) return 30
  return Math.max(1, Math.ceil(Math.abs(endTime - startTime) / 86400000) + 1)
}
...
export const generateOverviewData = (launchStart, launchEnd, spendStart, spendEnd, language = 'all', channel = 'all') => {
  ...
  const startDate = new Date(spendStart)
  const endDate = new Date(spendEnd)
  const diffTime = Math.abs(endDate.getTime() - startDate.getTime())
  const days = Math.max(1, Math.ceil(diffTime / 86400000) + 1)
  ...
  const createMetric = (label, baseValue, format, volatility, customRng) => {
    ...
    const history = Array.from({ length: days }, (_, index) => {
      ...
      return {
        ...
        value: Math.max(0, dailyBase * (1 + noise + spike)),
      }
    })
    const midpoint = Math.floor(history.length / 2)
    const recent = history.slice(midpoint).reduce((sum, item) => sum + item.value, 0)
    const previous = history.slice(0, midpoint).reduce((sum, item) => sum + item.value, 0)
    const trend = previous > 0 ? ((recent - previous) / previous) * 100 : 0
    const value = format === 'percent'
      ? history.reduce((sum, item) => sum + item.value, 0) / days
      : history.reduce((sum, item) => sum + item.value, 0)
    return { label, value, format, trend, history }
  }
...
export const generateTopMaterials = (launchStart, launchEnd, channel = 'all') => {
  const range = normalizeDateRange(launchStart, launchEnd)
  const rng = seedRandom(`${range.start}${range.end}${channel}`)
  const creators = ['苏雅', '顺子', '雅萱', '苗雪']
  const languages = ['EN', 'ZH', 'JP', 'KR', 'DE']
  ...
  for (let index = 0; index < goodCount; index += 1) {
    ...
    materials.push({
      id: `good-${index}`,
      name: `Core_Gameplay_Var_${index + 1}_Win`,
      thumbnail: `https://picsum.photos/270/480?random=${Math.floor(rng() * 1000) + index}`,
      cost,
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    materials.push({
      id: `bad-${index}`,
      name: `Static_Test_Fail_${index}`,
      thumbnail: `https://picsum.photos/270/480?random=${Math.floor(rng() * 1000) + 10 + index}`,
      cost,
      creator: creators[index % creators.length],
      launchDate,
...
const getFeedbackStatus = (cpi, ir, roasD7) => {
...
const getFeedbackInsight = (status, versionName) => {
...
export const generateFinishedCreativePerformance = (requirements = []) =>
  requirements.flatMap((requirement, requirementIndex) => {
...
export const summarizeRequirementFeedback = (performances = []) => {
...
export const summarizeDirectionFeedback = (performances = []) => {
  const grouped = performances.reduce((acc, item) => {
  ...
  return Object.entries(grouped).map(([scheduleId, items]) => {
    ...
    const status = winnerCount > 0 ? 'Winner' : failedCount >= Math.ceil(items.length / 2) ? 'Failed' : 'Learning'
...
const getDimensionCategories = (dimension) => {
  switch (dimension) {
...
export const generateCreativeAnalysisData = (
  ...
  const data = primaryCats.map((pCat) => {
    ...
    if (secondaryDim && secondaryCats.length > 0) {
      ...
      row.payerRate = Number(((totalPayersAccum / (totalCountAccum || 1)) * 100).toFixed(1))
...
export const generateMaterialDetails = (
  ...
  const launchDuration = Math.max(1, (eLaunch - sLaunch) / 86400000)
  ...
  const spendDuration = Math.max(1, (eSpend - sSpend) / 86400000)
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    rows.push({
      ...
      ctr: Number((rng() * 2).toFixed(2)),
      cvr: Number((rng() * 5).toFixed(2)),
      roas: Number((rng() * 1).toFixed(2)),
...
export const generatePersonnelData = (launchStart, launchEnd, spendStart, spendEnd, language = 'all', channel = 'all') => {
  const rng = seedRandom(`${launchStart}${launchEnd}${spendStart}${spendEnd}${language}${channel}`)
  const sSpend = new Date(spendStart).getTime()
  const eSpend = new Date(spendEnd).getTime()
  const spendDays = Math.max(1, (eSpend - sSpend) / 86400000)
  ...
  return baseData.map((person) => {
    const variability = 0.9 + rng() * 0.2
    const adjustedCost = Math.floor(person.baseCost * multiplier * variability * volMod)
    const share = (adjustedCost / (totalCost * multiplier * variability * volMod)) * 100
    const rate = Number((person.baseRate * variability).toFixed(2))
    const count = Math.floor(person.baseHigh * multiplier * variability * volMod * 2.5)
    const avgCost = Math.floor(adjustedCost / (count || 1))
    const efficiencyScore = Math.min(100, Math.floor((rate * 20000) / avgCost))
    return {
      name: person.name,
      costHigh: Math.floor(person.baseHigh * multiplier * variability * volMod),
      successRate: rate,
      successCost: adjustedCost,
      successCostShare: Number(share.toFixed(2)),
...
export const generatePersonnelHistory = (launchStart, launchEnd, spendStart, spendEnd, language = 'all', channel = 'all') => {
  ...
  const spendDays = Math.max(1, Math.ceil((eSpend.getTime() - sSpend.getTime()) / 86400000)) + 1
  ...
  for (let i = 0; i < spendDays; i += step) {
    ...
    personnel.forEach((name) => {
      ...
      point[`${name}_successRate`] = Number(newRate.toFixed(2))
      point[`${name}_costHigh`] = Math.floor(newCost / 500)
      dailyTotalCost += newCost
    })
    personnel.forEach((name) => {
      const cost = point[`${name}_successCost`]
      point[`${name}_successCostShare`] = Number((dailyTotalCost > 0 ? (cost / dailyTotalCost) * 100 : 0).toFixed(2))
    })
    history.push(point)
  }
  return history
