...
export const shiftUtcMonth = (date: Date, delta: number) =>
  new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth() + delta, 1));

export const getUtcMonthDays = (monthDate: Date) => {
...
export const isWithinDateRange = (date: string, start: string, end: string) =>
  Boolean(start && end && date >= start && date <= end);

export const isDateRangeEdge = (date: string, start: string, end: string) => date === start || date === end;
