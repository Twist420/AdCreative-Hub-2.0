...
export const getLeafFolders = (
  nodes: FolderNode[],
  path: string[] = [],
): { name: string; path: string[]; system: 'Fragment' | 'Component' }[] => {
...
export const getAllFoldersInTree = (
  nodes: FolderNode[],
  path: string[] = [],
...
export interface AssetFacetGroup {
  title: string;
  facets: AssetFacet[];
}

export interface AssetFacetCategory {
  id: string;
  label: string;
...
export const ASSET_FACET_CATEGORIES: AssetFacetCategory[] = [
  {
    id: 'popular',
    label: '热门推荐',
    groups: [
...
export const ASSET_FACETS = ASSET_FACET_CATEGORIES.flatMap(category =>
  category.groups.flatMap(group => group.facets)
);

export const getAssetUsageSlots = (item: LibraryItem): string[] => {
  const content = [item.type, item.subType, item.name, ...item.tags].join(' ').toLowerCase();
...
export const INITIAL_ITEMS: LibraryItem[] = [
  {
     id: 'fr-ai-01',
     type: 'Fragment',
     subType: 'AI前贴',
...
export const removeNodeAtPath = (nodes: FolderNode[], path: string[]): FolderNode[] => {
...
export const insertNodeAtPath = (nodes: FolderNode[], path: string[], newNode: FolderNode): FolderNode[] => {
  if (path.length === 0) {
...
export const updateNodeDetailsAtPath = (
  nodes: FolderNode[],
  path: string[],
...
export const getEffectivePerformance = (item: LibraryItem) => {
...
export const getMockCreatives = (item: LibraryItem) => {
  let seed = 0;
  for (let i = 0; i < item.id.length; i++) {
    seed += item.id.charCodeAt(i);
  }
