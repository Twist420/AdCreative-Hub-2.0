...
export type MaterialType = 'video' | 'playable' | 'image';
export type Language = 'EN' | 'JA' | 'KO' | 'DE';
...
export const platforms: SetItem['platform'][] = ['Android', 'iOS'];
export const languages: Language[] = ['EN', 'JA', 'KO', 'DE'];

const materialTypes: MaterialType[] = ['video', 'playable', 'image'];

export const buildMockSets = (): SetItem[] =>
...
export const INITIAL_COLUMNS: ColumnConfig[] = [
  ...
  { id: 'd7TotalRev', name: 'D7 total revenue', visible: true },
  { id: 'd0Roi', name: 'D0 total ROI', visible: true },
  { id: 'd7Roi', name: 'D7 total ROI', visible: true },
  { id: 'd7IapRev', name: 'D7 IAP revenue', visible: true },
  { id: 'd7IapRoi', name: 'D7 IAP ROI', visible: true },
  { id: 'd7Ret', name: 'D7 ret', visible: true },
...
export const metricHelp: Record<string, string> = {
  ...
  d7TotalRev: 'D7 total revenue：7日总收入。',
  d0Roi: 'D0 total ROI = D0模拟收入 / 花费。',
  d7Roi: 'D7 total ROI = D7总收入 / 花费。',
  d7IapRev: 'D7 IAP revenue：7日内购收入。',
  d7IapRoi: 'D7 IAP ROI = D7内购收入 / 花费。',
  d7Ret: 'D7 ret：第7日仍活跃用户占比。',
  d7Arppu: 'D7 ARPPU = D7总收入 / D7付费用户数。',
};

export const getMetrics = (row: SetItem) => {
...
export const getSortValue = (row: SetItem, key: string) => {
...
export const normalizeBenchmarkChannel = (channel: string) => {
  const normalized = channel.toLowerCase().replace(/\s+/g, '');
  if (normalized === 'applovin') return 'applovin_int';
  return normalized;
};

export const normalizeBenchmarkPlatform = (platform: string) => platform.toLowerCase();
...
export const getBenchmarkCellClassName = (metric: string, value: number, benchmark: BenchmarkRule) => {
  switch (metric) {
    case 'cpi':
